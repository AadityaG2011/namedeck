...
(function () {
  ...
  var DEMO_ROSTER = [
    ...
    { name: 'Rosa Parks',      wiki: 'Rosa_Parks' },
    { name: 'Frida Kahlo',     wiki: 'Frida_Kahlo' },
  ...
  function render() {
      ...
      '<div class="screen">' +
        '<div class="sheet" id="settings" hidden>' +
          ...
          '<div class="sheet-body">' +
            ...
            '</div>' +
            '<a class="privacy-link" id="privacyLink" href="privacy.html" target="_blank" rel="noopener">Privacy Policy</a>' +
          '</div>' +
    ...
    document.querySelector('#privacyLink').addEventListener('click', onPrivacyClick);
  ...
  var NATIVE_IMPORT_URL = 'https://aadityag2011.github.io/namedeck/native-import.html';
  var PRIVACY_URL = 'https://aadityag2011.github.io/namedeck/privacy.html';

  function isNative() {
    return !!(window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform());
  }

  // Open the privacy page. On the web the anchor's target=_blank handles it; natively we open
  // the system browser (with a Done button) so the app's own webview isn't navigated away.
  function onPrivacyClick(e) {
    if (isNative()) {
      e.preventDefault();
      try { window.Capacitor.Plugins.Browser.open({ url: PRIVACY_URL }); } catch (err) { /* ignore */ }
    }
  }
